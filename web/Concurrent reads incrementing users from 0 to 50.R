...
loadtest_mongodb <- "C:/User/E198869/Documents/MongoDB/MongoDB_MySQL_loadtests/1 Concurrent reads incrementing users from 0 to 50 - MongoDB.csv"
loadtest_mysql <- "C:/User/E198869/Documents/MongoDB/MongoDB_MySQL_loadtests/1 Concurrent reads incrementing users from 0 to 50 - MySQL.csv"
...
data_mongodb <- aggregate(data_mongodb_df$rt, by = list(ts=data_mongodb_df$ts), mean)
data_mysql <- aggregate(data_mysql_df$rt, by = list(ts=data_mysql_df$ts), mean)
...
plot(rt ~ second,
	...
	type="p",
	col="darkblue")
title(main="Concurrent reads incrementing users from 0 to 50")
mtext("Incrementing by five each five seconds. Each thread is kept for 100 seconds", line=0.5, cex=0.8)
